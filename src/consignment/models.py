...
class Consignment(Support):
    ...
    schedule_shomen = models.CharField(
        "書面審査時期", max_length=50, blank=True, null=True)
    schedule_mensetsu = models.CharField(
        "面接審査時期", max_length=50, blank=True, null=True)
    call_for_application = models.CharField(
        "公募名", max_length=200, blank=True, null=True
    )
    funding_agency = models.CharField(
        "配分機関", max_length=50, blank=True, null=True)
